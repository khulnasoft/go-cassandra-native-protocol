...
type CqlClient struct {
	...
	// This is only useful when using continuous paging, a feature specific to KhulnaSoft Enterprise.
	MaxPending int
...
func (c *CqlClientConnection) NewStartupRequest(version primitive.ProtocolVersion, streamId int16) (*frame.Frame, error) {
	...
	startup.SetDriverName("KhulnaSoft Go client")
...
type InFlightRequest interface {
	...
	// only ever emit one single frame, except when using continuous paging (KhulnaSoft Enterprise only).
