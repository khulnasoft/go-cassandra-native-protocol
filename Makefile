...
check-license: | $(ADDLICENSE) ; $(info $(M) checking license headers...) @ ## Checks license headers in all source files
	$Q $(ADDLICENSE) -check -c "KhulnaSoft" -ignore '.github/**'  -ignore '.idea/**' -ignore '$(BUILD)/**' . 2> /dev/null

update-license: | $(ADDLICENSE) ; $(info $(M) updating license headers...) @ ## Updates license headers in all source files
	$Q $(ADDLICENSE) -c "KhulnaSoft" -ignore '.github/**'  -ignore '.idea/**' -ignore '$(BUILD)/**' . 2> /dev/null
...
deep-copy: | $(GENGO_DIR) $(DEEPCOPY) ; $(info $(M) generating deepcopy methods...) @ ## Generates DeepCopy methods
	$Q $(DEEPCOPY) -h deepcopy-header.txt -i github.com/khulnasoft/go-cassandra-native-protocol/primitive --trim-path-prefix github.com/khulnasoft/go-cassandra-native-protocol -o .
	$Q $(DEEPCOPY) -h deepcopy-header.txt -i github.com/khulnasoft/go-cassandra-native-protocol/message   --trim-path-prefix github.com/khulnasoft/go-cassandra-native-protocol -o .
	$Q $(DEEPCOPY) -h deepcopy-header.txt -i github.com/khulnasoft/go-cassandra-native-protocol/datatype  --trim-path-prefix github.com/khulnasoft/go-cassandra-native-protocol -o .
	$Q $(DEEPCOPY) -h deepcopy-header.txt -i github.com/khulnasoft/go-cassandra-native-protocol/frame     --trim-path-prefix github.com/khulnasoft/go-cassandra-native-protocol -o .
	$Q $(DEEPCOPY) -h deepcopy-header.txt -i github.com/khulnasoft/go-cassandra-native-protocol/segment   --trim-path-prefix github.com/khulnasoft/go-cassandra-native-protocol -o .
